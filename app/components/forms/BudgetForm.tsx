...
export default function BudgetForm({
  ...
  const [form, setForm] = useState({
    ...
    type: types[1],
    date: '',
    color: '#f59e0b', // ✅ couleur par défaut pour les budgets
  });
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmedTitre || !form.amount || !form.date || !form.type || !category) {
    ...
    try {
      const payload = {
        ...
        type: form.type,
        date: form.date,
        color: '#f59e0b', // ✅ forcée à chaque envoi
      };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 max-w-lg mx-auto space-y-5 border border-pink-100">
      ...
      <div>
        ...
        >
          {types.map((t) => (
            <option key={t} value={t}>{t === 'revenu' ? 'Revenu' : 'Dépense'}</option>
          ))}
        </select>
      </div>
